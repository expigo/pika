...
**Test Coverage (as of July 1 2026 — see `docs/TEST_AUDIT_2026_06_30.md`):** ~1,071 passing JS/TS
tests + 46 gated DB-integration + 8 Python sidecar.
- Desktop: 443 tests (Vitest; unit + `*.rtl.tsx` React-Testing-Library component tests, +1 skipped)
- Cloud: ~418 unit tests (Bun) + **46** real-Postgres integration (`test:integration`, gated `RUN_DB_TESTS` —
...
- Shared: 43 tests (Bun)
- Web: 94 pure tests (`bun test`) + 71 `*.rtl.tsx` RTL component tests (`vitest run`) = 165. **Dual-runner** (see [RTL harness](#rtl-component-tests-web--desktop) below)
...
### Testing Philosophy
- **~1,071 passing tests** across all packages (443 desktop, ~418 cloud, 165 web, 43 shared) + 46 gated
