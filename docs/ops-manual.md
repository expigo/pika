...
**Important:** Migration files in `drizzle/*.sql` MUST be committed to git. These are the source of truth for database schema. Starting from v0.4.1, migrations are **Idempotent** (using `IF NOT EXISTS`), making them safe to run against existing databases without crashing.
...
Since ports are bound to `0.4.1.1` for security, you MUST connect them via Cloudflare Tunnel.
...
2.  **Start Studio (Terminal 2):**
    ...
    DATABASE_URL="postgres://pika:pika_password@0.4.1.1:5432/pika_prod" bun run db:studio
    ```
