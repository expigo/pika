...
**Important:** Migration files in `drizzle/*.sql` MUST be committed to git. These are the source of truth for database schema. Starting from v0.4.0, migrations are **Idempotent** (using `IF NOT EXISTS`), making them safe to run against existing databases without crashing.
