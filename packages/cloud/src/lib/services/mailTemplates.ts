/**
 * The message layer — branded shells, template senders (magic link / deletion / OTP, Night
 * Recap / DJ digest), and the throttled send orchestration (auth + marketing wrappers).
 * Split (2026-07) from mail.ts, behavior-preserving; mail.ts keeps the transport core.
 *
 * The WHOLE orchestration layer lives here with the templates on purpose: `defaultAuthMailDeps`
 * references the transactional senders, so splitting the throttle wrappers from the templates
 * would create a mail ⇄ templates import cycle. Dependency is strictly one-way: this file →
 * mail.ts (sendEmail/deps/errors) + email-throttle.ts (budget primitives).
 *
 * DI-style like journal.ts (tests inject deps; never mock.module).
 */

import { logger } from "@pika/shared";
import { type EmailThrottle, mailThrottle, marketingMailThrottle } from "./email-throttle";
import { defaultMailDeps, type MailDeps, MailThrottledError, maskEmail, sendEmail } from "./mail";
