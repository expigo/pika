 ...
 *
 * Split (2026-07): the service-account + playlist layer lives in ./spotifyPlaylist.ts; this file
 * keeps the per-DJ OAuth, now-playing, and app-token core, exports the shared primitives the
 * playlist module needs, and re-exports the moved symbols as a compatibility facade (bottom).
 */
...
export const ACCESS_TOKEN_SKEW_MS = 30_000; // refresh this long before expiry
...
export function getConfig(): { clientId: string; clientSecret: string; redirectUri: string } {
...
export async function fetchSpotifyUserId(accessToken: string): Promise<string | undefined> {
  try {
...
// ---------------------------------------------------------------------------
// Compatibility facade (2026-07 split): the service-account + playlist layer
// moved to ./spotifyPlaylist.ts. Existing importers keep working unchanged;
// NEW code should import from "./spotifyPlaylist" directly.
// ---------------------------------------------------------------------------
