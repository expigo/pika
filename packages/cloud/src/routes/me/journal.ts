/**
 * Journal routes (Slice B) — `/api/me/journal*`: claim this device's clientId for the account,
 * the union journal read, device unlink, account-scoped unlike, and the account playlist export.
 *
 * Auth: the composer (`../me.ts`) applies `requireAuth` to every /api/me route BEFORE the
 * mounts; CSRF (`X-Pika-Client`) is applied at the /api/me mount in index.ts.
 */
 ...
 */
journalRoutes.post("/journal/claim", zValidator("json", ClaimBody), async (c) => {
 ...
 */
journalRoutes.delete("/journal/devices/:clientId", async (c) => {
 ...
 */
journalRoutes.delete("/journal/likes/:likeId", async (c) => {
  const userId = getUser(c).id;
 ...
 */
journalRoutes.post("/journal/playlist", async (c) => {
  const userId = getUser(c).id;
  try {
