 ...
 *
 * Exercises the branches that run BEFORE any DB call: zod body validation and
 * the unauthenticated (no Bearer header) 401 path — `requireDj` returns null
 * before touching the DB when there is no token. DB-touching paths (create
...
describe("stage routes — event creation validation (real handler)", () => {
  ...
  test("rejects a valid body with no auth token (401, before any DB call)", async () => {
    ...
    expect(((await res.json()) as ApiResponse).error).toBe("Unauthorized");
  });
