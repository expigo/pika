...
function generateCsv(session: Session, plays: PlayWithTrack[]): string {
    ...
    const rows = plays.map((play, index) => [
        ...
        `"${(play.artist || "Unknown").replace(/"/g, '""')}"`,
        `"${(play.title || "Unknown").replace(/"/g, '""')}"`,
        play.bpm?.toFixed(1) || "",
        play.key || "",
        play.reaction,
        `"${(play.notes || "").replace(/"/g, '""')}"`,
        play.dancerLikes,
    ]);
                ...
                <div style={styles.sessionList}>
                    {sessions.map((session) => (
                        <div
                            ...
                            style={{
                                ...styles.sessionItem,
                                ...(selectedSessionId === session.id && !showSummary ? styles.sessionItemActive : {}),
                            }}
                        >
                            <button
                                type="button"
                                    ...
                                    {sessionDetails.plays.map((play, index) => (
                                        ...
                                        >
                                            ...
                                            <td style={styles.td}>
                                                ...
                                                {play.reaction === "brick" && (
                                                    <BrickWall size={16} color="#64748b" />
                                                )}
                                            </td>
                                            <td style={styles.td}>
                                                {play.dancerLikes > 0 && (
