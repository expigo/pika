...
export const savedSetRepository = {
    ...
    async updateSetTracks(setId: number, trackIds: number[]): Promise<void> {
        ...
        await sqlite.execute(
            `DELETE FROM saved_set_tracks WHERE set_id = $1`,
            [setId]
        );
        ...
        await sqlite.execute(
            `UPDATE saved_sets SET updated_at = $1 WHERE id = $2`,
            [now, setId]
        );
    },
    ...
    async renameSet(setId: number, name: string): Promise<void> {
        ...
        await sqlite.execute(
            `UPDATE saved_sets SET name = $1, updated_at = $2 WHERE id = $3`,
            [name, now, setId]
        );
    },
    ...
    async deleteSet(setId: number): Promise<void> {
        ...
        // Delete the set
        await sqlite.execute(
            `DELETE FROM saved_sets WHERE id = $1`,
            [setId]
        );
    },
};
