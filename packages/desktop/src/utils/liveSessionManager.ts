...
import { normalizeForMatch, type ParsedTrack, parseVdjM3u } from "./m3uParser";
...
async function pollHistoryFolder(folderPath: string): Promise<ParsedTrack[]> {
  ...
  try {
    ...
    const sessionTracks = allTracks.filter((t) => t.timestamp >= state.sessionStart);

    // Filter: Only new tracks (not already processed)
    const newTracks = sessionTracks.filter((t) => !state.processedTimestamps.has(t.timestamp));
...
async function findTrackByArtistTitle(artist: string, title: string): Promise<Track | null> {
  ...
  match = allTracks.find((t) => normalizeForMatch(t.title || "") === normalizedTitle);
...
async function processNewPlays(folderPath: string): Promise<void> {
  ...
  for (const parsedTrack of newTracks) {
    ...
    let dbTrack = await findTrackByArtistTitle(parsedTrack.artist, parsedTrack.title);

    if (!dbTrack) {
      // Create ghost track for unknown songs
      dbTrack = await createGhostTrack(parsedTrack.artist, parsedTrack.title);
      console.log(
        `[LiveSession] Created ghost track: ${parsedTrack.artist} - ${parsedTrack.title}`,
      );
    }
 ...
 */
export async function manuallyAddPlay(trackId: number, timestamp?: number): Promise<void> {
  if (!state.sessionId) {
